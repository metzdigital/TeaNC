...
(function($, window, document, undefined) {
	...
	var pluginName = 'rotaryswitch',
		defaults = {
			...
			originDeg: 0, // Start point in deg
	...
	function Plugin(element, options) {
		...
		this.htmlStructure = {
			...
			switchButton:  '<div class="rotary_switch"></div>',
	...
	Plugin.prototype = {
      ...
      // Apply zoom to css:
      this.domElements.main.css({'zoom': this.options.zoom});
		...
		*/
		calculateSwitchDeg: function() {
			var offset =  this.domElements.main.offset(),
				radians = Math.atan2(this.mousePosition.x - (offset.left*this.options.zoom + (this.domElements.main.width()*this.options.zoom/2)), this.mousePosition.y - (offset.top*this.options.zoom + (this.domElements.main.height()*this.options.zoom/2)));
		...
		*/
		calculateValueByDeg: function() {
			...
			if (this.switchDeg - this.options.originDeg > 0) {
				this.valueInPercent = (this.switchDeg - this.options.originDeg)  / this.totalDeg;
			} else {
				this.valueInPercent = (this.switchDeg - this.options.originDeg + 360)  / this.totalDeg;
			}
		...
		*/
		rotateSwitch: function(snap) {
			...
			if (snap === true || (this.options.snapInMotion === true && difference < this.degPerStep / 6)) {
				if (roundedDeg + this.options.originDeg < 360) {
					deg = (roundedDeg + this.options.originDeg);
				} else {
					deg = roundedDeg + this.options.originDeg - 360;
				}
			} else {
				if (exactDeg + this.options.originDeg < 360) {
					deg = (exactDeg + this.options.originDeg);
				} else {
					deg = exactDeg + this.options.originDeg - 360;
				}
			}
